...
def main() -> None:
    ...
    for tkr, g in base.groupby("Ticker", sort=False):
        ...
        success = (fut_max_high >= close * (1.0 + pt)).astype("float")  # keep float then later dropna

        # 보조: 미래 최저저점 기준 미래DD(추후 tail 라벨 등에도 유용)
        fut_dd = (fut_min_low / close) - 1.0

        g["Success"] = success
        g["FutureMinDD"] = fut_dd

        # horizon 부족한 구간은 NaN -> 제거
    ...
    # also write canonical training csv
