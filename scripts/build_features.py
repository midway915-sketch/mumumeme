...
# ✅ sys.path guard: allow `python scripts/build_features.py` to import `scripts.*`
import sys
...
def compute_ticker_features(g: pd.DataFrame, market_ret_by_date: pd.Series) -> pd.DataFrame:
    # IMPORTANT: reset_index so rolling/cov alignment is deterministic and
    # all arrays used to build the output DataFrame have identical length.
    ...
    r = c.pct_change()
    # Force mret to share the SAME row index as r; otherwise pandas aligns by index labels
    # and can produce misaligned outputs -> "All arrays must be of the same length".
    mret_vals = market_ret_by_date.reindex(dt).to_numpy(dtype=float)
    mret = pd.Series(mret_vals, index=g.index, dtype=float)
...
def add_sector_strength(feats: pd.DataFrame, ticker_to_group: dict[str, str]) -> pd.DataFrame:
    # Sector features are REQUIRED when enabled.
    if not ticker_to_group:
        raise ValueError(
            "Sector features requested but universe.csv Group mapping is missing/empty. "
            "Fix: add a 'Group' column to data/universe.csv (sector name per ticker)."
        )

    if "ret_20" not in feats.columns:
        raise ValueError(
            "Sector features requested but features are missing 'ret_20'. "
            "Fix: ensure build_features computes ret_20."
        )
...
def main() -> None:
    ...
    if enable_sector:
        ...
        # hard guarantee
        for c in ["Sector_Ret_20", "RelStrength"]:
            if c not in feats.columns:
                raise RuntimeError(f"Sector feature '{c}' was not created (unexpected).")

    # Sector is mandatory in this project (unless explicitly disabled)
    sector_enabled = bool(enable_sector)

    # ✅ SSOT feature set (includes sector cols when enabled)
