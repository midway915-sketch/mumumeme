...
LOOKAHEAD_SHIFT_DAYS = 1  # B(당일 종가 진입) 기준: 피처는 무조건 1일 lag
...
def read_universe_groups_strict() -> tuple[dict[str, str], list[str]]:
    ...
    if not tickers:
        raise ValueError("universe.csv tickers empty (after Enabled filter).")
...
def compute_ticker_features(g: pd.DataFrame, market_ret_by_date: pd.Series) -> pd.DataFrame:
    ...
    # beta_60 (index 정렬)
    r = c.pct_change()
    mret_np = market_ret_by_date.reindex(dt).to_numpy(dtype=float)
    mret = pd.Series(mret_np, index=g.index)
    ...
    bad = {k: v for k, v in bad.items() if v != n}
    if bad:
...
def main() -> None:
    ...
    # ✅ 피처는 유니버스 티커만 생성 (SPY는 시장 피처 전용)
    feats_list: list[pd.DataFrame] = []
    px_trade = prices.loc[prices["Ticker"].isin(set(universe_tickers))].copy()

    for _t, g in px_trade.groupby("Ticker", sort=False):
    ...
    if feats.empty:
        raise RuntimeError("No features produced. Check prices/universe tickers.")
    ...
    if len(feats):
        ...
        print(f"[INFO] universe_tickers: {len(universe_tickers)} (features built only for universe)")
