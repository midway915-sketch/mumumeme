...
LBL_PARQ = DATA_DIR / "labels" / "labels_tail.parquet"
LBL_CSV = DATA_DIR / "labels" / "labels_tail.csv"

APP_DIR = Path("app")
MODEL_PATH = APP_DIR / "tail_model.pkl"
SCALER_PATH = APP_DIR / "tail_scaler.pkl"
META_PATH = APP_DIR / "tail_model_meta.json"
...
def pick_feature_cols(df: pd.DataFrame) -> list[str]:
    # drop identifiers + raw columns that can leak/overfit (Close is still okay, but you already use it downstream)
    drop = {"Date", "Ticker"}
    # keep everything numeric except identifiers
    cols = []
    for c in df.columns:
        if c in drop:
            continue
...
def main() -> None:
    ...
    if "TailTarget" not in lbl.columns:
        raise ValueError("labels_tail must have TailTarget")

    df = feats.merge(lbl[["Date", "Ticker", "TailTarget"]], on=["Date", "Ticker"], how="inner")
    df = df.dropna(subset=["TailTarget"]).copy()
    ...
    y = pd.to_numeric(df["TailTarget"], errors="coerce").fillna(0).astype(int).to_numpy()
    ...
    if len(Xte):
        ...
        # AUC may fail if only one class
        try:
            from sklearn.metrics import roc_auc_score
            auc = float(roc_auc_score(yte, p))
        except Exception:
            auc = float("nan")
        print(f"[INFO] tail_model test AUC={auc:.4f} (may be NaN if degenerate)")
